...
const PreBookOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const res = await axios.get(
        `${url}api/orders/pre-book?page=${page}&limit=${limit}`,
        {
      ...
      const totalCount = res.data?.total || res.data?.count || receivedItems.length;
      ...
      setMetaSummary({
        ...
        uniqueCustomers:
          res.data?.metaSummary?.uniqueCustomers || res.data?.uniqueCustomers || 0,
        itemsSold: res.data?.metaSummary?.itemsSold || res.data?.itemsSold || 0,
      });
    } catch (err) {
      console.error("fetchOrders error:", err);
      toast.error(err?.response?.data?.message || "Failed to fetch pre-book orders");
      setOrders([]);
    } finally {
      setLoading(false);
    }
  ...
  const stats = useMemo(() => {
    const amount =
      metaSummary.totalSales ||
      orders.reduce((sum, o) => sum + (Number(o?.totalPrice) || 0), 0);
    const customers =
      metaSummary.uniqueCustomers ||
      new Set(orders.map((o) => o?.customer?.mobile || "")).size;
    const qty =
      metaSummary.itemsSold ||
      orders.reduce(
        (sum, o) =>
          sum +
          (o.products || []).reduce(
  ...
  const confirmOrder = async (orderId) => {
    try {
      ...
      toast.success("Pre-book order confirmed (processing)");
      await fetchOrders();
      closeDetails();
    } catch (e) {
  ...
  const cancelOrder = async (orderId) => {
    try {
      ...
      toast.success("Pre-book order cancelled and booked count restored");
      await fetchOrders();
      closeDetails();
    } catch (e) {
  ...
  return (
    <div className="px-1 py-4 bg-slate-50 min-h-screen w-full font-sans text-slate-800 relative">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-gradient-to-br from-slate-50 via-white to-blue-50/40" />
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-blue-100 text-blue-700 px-2.5 py-0.5 text-xs font-bold">
              <FiBookmark className="w-3 h-3" /> Pre-book
            </span>
          </div>
          <p className="text-sm text-slate-500">
            Monitor, verify and process pre-order queue and delivery schedules.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 w-full lg:w-auto">
          <div className="relative flex-1 lg:flex-none">
            ...
            <input
              ...
              placeholder="Search pre-book orders..."
              className="w-full lg:w-80 bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-2 text-xs font-medium focus:outline-none focus:border-blue-500 shadow-sm"
            />
          </div>
      ...
      <div className="mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard
          icon={<FiBookmark />}
          label="Total Pre-books"
          value={stats.orderCount.toLocaleString()}
          subtitle="Orders queued"
          color="bg-blue-50 text-blue-600"
        />
        <StatCard
          icon={<FiDollarSign />}
          label="Pre-book Value"
          value={fmtBDT(stats.totalAmount)}
          subtitle="Estimated volume"
          trend={true}
          color="bg-emerald-50 text-emerald-600"
        />
        <StatCard
          icon={<FiUser />}
          label="Pre-book Clients"
          ...
          <table className="min-w-full text-xs font-medium">
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-100 uppercase tracking-wider text-[10px] font-bold">
              <tr>
                ...
                <th className="px-6 py-4 text-center">Expected Delivery</th>
            ...
            <tbody className="divide-y divide-slate-100 font-sans text-slate-700">
              {loading ? (
                <tr>
                  <td
                    colSpan={9}
                    className="px-6 py-12 text-center text-slate-400 font-semibold animate-pulse"
                  >
                    Syncing pre-book orders...
                  </td>
                </tr>
              ) : (
                filteredOrders?.map((order) =>
                  (order.products || []).map((item, pi) => {
                    ...
                    const productImg =
                      ...
                      null;
                    const expectedDate =
                      item?.product?.preBook?.expectedDeliveryDate;
                    ...
                    return (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="max-w-[180px]">
                              ...
                              <span className="text-[10px] text-slate-400 block mt-0.5 flex items-center gap-1">
                                ...
                                {fmtBDDateTime(order?.createdAt)}
                              </span>
                            </div>
                          </div>
                        </td>

                        <td className="px-6 py-4 align-middle text-slate-400 font-mono font-semibold">
                          {item?.product?.sku || "—"}
                        </td>
                        <td className="px-6 py-4 align-middle text-center font-bold text-slate-800">
                          {item?.quantity || 0}
                        </td>
                        <td className="px-6 py-4">
                          <div>
                            <div className="font-bold">
                              {fmtBDT(item?.finalPrice || item?.price)}
                            </div>
                            {(item?.discountAmount || 0) > 0 && (
                              <div className="text-[10px] text-green-600">
                                Save {fmtBDT(item?.discountAmount)}
                              </div>
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 align-middle text-center">
                          {expectedDate ? (
                            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-blue-700 bg-blue-50 border border-blue-200/60 px-2 py-0.5 rounded-md">
                        ...
                        <td className="px-6 py-4 align-middle text-center">
                          <span
                            className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-bold capitalize ${orderStatusBadgeClass(
                              oStatus
                            )}`}
                          >
                            {oStatus}
                        ...
                        <td className="px-6 py-4 align-middle text-center">
                          <span
                            className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-bold capitalize ${paymentBadgeClass(
                              pStatus
                            )}`}
                          >
                            {pStatus}
                        ...
                        <td className="px-6 py-4 align-middle text-right">
                          {showActionOnce ? (
                            ...
                            >
                              <FiEye className="text-blue-600 w-3.5 h-3.5" /> View
                            </button>
                          ) : (
              ...
              {!loading && filteredOrders.length === 0 && (
                <tr>
                  ...
                  >
                    <FiBookmark className="w-8 h-8 text-slate-300 mx-auto mb-2" />
        ...
        {stats.orderCount > 0 && (
          <div className="bg-slate-50/70 border-t border-slate-100 px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 font-sans text-xs font-bold text-slate-500">
            <div>
              ...
              <span className="text-slate-800">{stats.orderCount}</span> pre-book orders
            </div>
      ...
      <div className="relative bg-white rounded-2xl max-w-3xl w-full shadow-2xl border border-slate-100 transform transition-all flex flex-col font-sans max-h-[88vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-semibold mt-0.5">
              Unique ID:{" "}
              <span className="font-mono text-blue-600 font-bold">{order?._id}</span>
            </p>
          </div>
          <button
            onClick={onClose}
            className="h-8 w-8 grid place-items-center rounded-lg hover:bg-slate-50 text-slate-400 hover:text-slate-600"
          >
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 space-y-1">
              ...
              <p className="text-xs text-slate-500 font-bold flex items-center gap-1.5">
                <FiPhone className="text-slate-400 w-3.5 h-3.5" />{" "}
                {order?.customer?.mobile || "—"}
              </p>
              <p className="text-[11px] text-slate-400 pt-1 flex items-center gap-1.5">
                <FiClock className="text-slate-400 w-3.5 h-3.5" /> Created:{" "}
                {fmtBDDateTime(order?.createdAt)}
              </p>
            </div>
            ...
            <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 space-y-1">
              ...
              <p className="text-xs text-slate-700 font-semibold flex items-center gap-1.5">
                <FiTruck className="text-slate-400 w-3.5 h-3.5" /> Option:{" "}
                <span className="font-bold text-slate-900 ml-1">
                  {order?.shippingOption || "—"}
                </span>
              </p>
              <p className="text-xs text-slate-700 font-semibold flex items-center gap-1.5">
                <FiDollarSign className="text-slate-400 w-3.5 h-3.5" /> Charge:{" "}
                <span className="font-bold text-slate-900 ml-1">
                  {fmtBDT(order?.shippingCost || 0)}
                </span>
              </p>
              <p className="text-[11px] text-slate-500 font-medium pt-1 flex items-start gap-1.5">
                <FiMapPin className="text-slate-400 w-3.5 h-3.5 mt-0.5 shrink-0" />{" "}
          ...
          <div className="rounded-xl border border-slate-100 overflow-hidden shadow-sm">
            <div className="bg-slate-50 px-4 py-2.5 text-[10px] font-bold text-slate-500 uppercase tracking-wider border-b border-slate-100">
              Pre-booked Product Queue
            </div>
            <div className="divide-y divide-slate-100 max-h-48 overflow-y-auto">
              {(order.products || []).map((it, idx) => {
                ...
                const expectedDelivery =
                  it?.product?.preBook?.expectedDeliveryDate;

                return (
                  <div
                    key={idx}
                    className="p-3.5 flex gap-3 items-center bg-white"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-slate-400">
                        Original: {fmtBDT(it?.originalPrice || unit)}
                      </p>
                      {(it?.discountAmount || 0) > 0 && (
                        <p className="text-[11px] text-green-600">
                          Discount: -{fmtBDT(it?.discountAmount)}
                        </p>
                      )}
                      <p className="text-[11px] font-bold text-slate-700">
                        Final: {fmtBDT(it?.finalPrice || unit)}
                      </p>
                    </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 pt-2 border-t border-slate-100">
            <div className="text-xs font-bold text-slate-500 flex items-center gap-1.5">
              Pipeline Stage:{" "}
              <span className="bg-blue-50 text-blue-700 px-2.5 py-0.5 rounded-md uppercase text-[10px] font-black border border-blue-100">
            ...
            <div className="w-full sm:w-60 space-y-1.5 text-xs font-bold text-slate-400 text-right">
              ...
              {(order?.discountAmount || 0) > 0 && (
                <div className="flex justify-between text-emerald-600">
                  <span>Coupon Save ({order.couponCode}):</span>
                  <span className="font-black">
                    - {fmtBDT(order?.discountAmount || 0)}
                  </span>
                </div>
              )}
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 rounded-b-2xl flex flex-col sm:flex-row gap-2 sm:justify-end">
          <button
            onClick={onClose}
            className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-2 text-xs font-bold text-slate-600 bg-white hover:bg-slate-50 shadow-sm"
          >
            Close
          </button>
          <button
            ...
            onClick={onCancel}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-100 px-4 py-2 text-xs font-bold disabled:opacity-40 shadow-sm"
          >
            <FiXCircle /> Cancel Pre-book
          </button>
          <button
            disabled={!canConfirm || busy}
            onClick={onConfirm}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 text-xs font-bold disabled:opacity-40 shadow-md shadow-blue-100"
          >
